...
import edu.scut.luluteam.serialportlibrary.thread.SerialPortReadThread;
...
public class SerialPortManager extends SerialPort {
    ...
    public boolean openSerialPort(File device, int baudRate) {
        ...
        Log.i(TAG, "openSerialPort: " + String.format("打开串口 %s  波特率 %d", device.getPath(), baudRate));

        // 校验串口权限
        if (!device.canRead() || !device.canWrite()) {
            boolean mod666 = chmod666(device);
